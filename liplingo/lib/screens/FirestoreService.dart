...
class FirestoreService {
  static Future<void> initializeUserLevels(String userEmail) async {
    final docRef = FirebaseFirestore.instance.collection('Completed_Challenges').doc(userEmail);
  ...
  static Future<void> updateLevelProgress(int completedLevelNumber, int starsEarned) async {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      levelsData[levelKey] = {
        'isLocked': false, // The current level is obviously unlocked since it's completed
        'numberOfStars': starsEarned, // Update the stars earned for this level
      };
      ...
      if (completedLevelNumber < 12) {
        ...
        nextLevelData['numberOfStars'] = nextLevelData['numberOfStars'] ?? 0; // Keep stars at 0 for an unlocked level
